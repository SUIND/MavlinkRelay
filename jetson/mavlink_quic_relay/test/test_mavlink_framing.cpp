...
TEST(MavlinkFramingTest, Encode_MaxFrameSize) {
  ...
  const uint16_t decoded_len =
      static_cast<uint16_t>(encoded[0]) | (static_cast<uint16_t>(encoded[1]) << 8);
...
TEST(MavlinkFramingTest, Encode_EmptyPayload_Throws) {
  EXPECT_THROW(encodeFrame({}), std::invalid_argument);
}
...
TEST(MavlinkFramingTest, RoundTrip_EncodeDecode) {
  const std::vector<uint8_t> heartbeat =
      {0xFD, 0x09, 0x00, 0x00, 0x01, 0x01, 0xC1, 0x00, 0x00, 0x00, 0x00};
...
TEST(MavlinkFramingTest, RoundTrip_V1Heartbeat) {
  const std::vector<uint8_t> v1_heartbeat =
      {0xFE, 0x09, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x08, 0x00, 0x00, 0x03};
