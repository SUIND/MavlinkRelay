...
ReconnectManager::ReconnectManager(QuicClient& client, RosInterface& ros_iface)
    : quic_client_(client), ros_iface_(ros_iface), rng_(std::random_device{}())
{
}
...
ReconnectManager::State ReconnectManager::state() const noexcept { return state_.load(std::memory_order_relaxed); }
  ...
  {
    {
      ...
      reconnect_cv_.wait(lock, [this] { return should_reconnect_.load() || !running_.load(); });
    ...
    ROS_INFO("mavlink_quic_relay: attempting reconnect (attempt %d, delay was %dms)", reconnect_attempts_, delay_ms);
