...
BoundedQueue::BoundedQueue(std::size_t max_size) : max_size_(max_size) {}
...
{
  mavlink_sub_ =
      nh_.subscribe<mavros_msgs::Mavlink>(config_.mavlink_from_topic, 100, &RosInterface::mavlinkFromCallback, this);

  mavlink_pub_ = nh_.advertise<mavros_msgs::Mavlink>(config_.mavlink_to_topic, 100);

  drain_timer_ = nh_.createTimer(config_.drain_period, &RosInterface::drainInboundCallback, this);

  node_start_time_ = ros::Time::now();
}

std::optional<MavlinkFrame> RosInterface::popOutbound() { return outbound_queue_.tryPop(); }

void RosInterface::pushInbound(MavlinkFrame frame) { inbound_queue_.push(std::move(frame)); }

void RosInterface::clearOutboundQueue() { outbound_queue_.clear(); }
