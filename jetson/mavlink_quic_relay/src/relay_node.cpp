...
RelayNode::RelayNode(ros::NodeHandle& nh, RelayNodeConfig config)
    ...
    , nh_(nh)
{
    ...
    quic_client_->setFrameReceivedCallback(
        [this](std::vector<uint8_t> frame) {
            onFrameReceived(std::move(frame));
        });

    quic_client_->setConnectionStateCallback(
        [this](bool connected) {
    ...
    quic_event_timer_ = nh_.createTimer(
        ros::Duration(0.001),
        &RelayNode::quicEventCallback,
        this);
}
...
void RelayNode::start()
{
    ...
    ROS_INFO("mavlink_quic_relay: relay started, connecting to %s:%d",
             config_.quic.server_host.c_str(),
...
void RelayNode::quicEventCallback(const ros::TimerEvent& /*event*/)
{
    quic_client_->processEvents();
}
...
void RelayNode::onAuthOk()
{
    reconnect_manager_->onAuthOk();
}

void RelayNode::onAuthFailed()
{
    reconnect_manager_->onAuthFailed();
}
...
void RelayNode::onFrameReceived(std::vector<uint8_t> frame)
{
    ...
    if (frame.size() >= 8U) {
        if (frame[0] == 0xFDU) {
            // MAVLink v2
            msgid = static_cast<uint32_t>(frame[7])
